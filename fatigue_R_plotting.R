...
setwd('/Users/joshuagantner/Library/CloudStorage/OneDrive-UniversitätZürichUZH/Files/Studium/Masterarbeit/0 v9/figures_draft_072023')
...
# 1. PLOT VARIABILITY MODELS
## (a) 2 day model
##### build data frame of reapplied 2 day model -->
...
## (b) 1 day models
### training day
##### build data frame of reapplied model -->
tableD1 <- data.frame(
  time = c(0, 0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4),
  CON = c(31.34, 29.81, 28.28, 26.75, 25.22, 23.69, 22.16, 20.63, 19.10),
  FSD = c(29.48, 29.70, 29.93, 30.16, 30.38, 30.61, 30.84, 31.06, 31.29),
  FRD = c(37.51, 36.58, 35.64, 34.70, 33.76, 32.83, 31.89, 30.95, 30.02)
)
##### Reshape data to long format -->
...
# 4. PLOT MAX EMG AMPLITUDE
##### build data frame -->
maxampD1 <- data.frame(
  time = c(0, 1, 2, 3, 4),
  CON = c(0.180129693, 0.165682820, 0.151235947, 0.136789074, 0.122342201),
  fatigued = c(0.170605085, 0.165917880, 0.161230675, 0.156543470, 0.151856265)
)
