...
disp(' ')
disp(' ')
disp(' 98 load all_dat')
disp(' 99 save all_dat')
...
%Case 3: Create fatigue_alldat
    case 3
        ...
        time_start = now;
        %create alldat
        for i = 1:3 %length(Parameters.SessN)
...
%Cas 6: stnd4time
    case 6
        ...
        disp("  -> Trials standardized for Tim")
        disp(strcat("     runtime: ",datestr(now-start_time,"MM:SS")))
        close(h)
...
%Case 8: Calculate Variables
    case 8
        ...
        for i = leads
           for j = vars
               fatigue_alldat.(strcat(i,"_",j)) = zeros(l,1);
           end
        end
        ...
        for i = 1:length(fatigue_alldat.SubjN)
            ...
            for j = leads
                ...
                %get trial_mean
                trial_mean = mean_trials.(strcat(j,"_mean"))(mean_trials.subjn  == fatigue_alldat.SubjN(i) &...
                                                             mean_trials.day    == fatigue_alldat.day(i) &...
                                                             mean_trials.BN     == fatigue_alldat.BN(i));
                trial_mean = trial_mean{1};
                
                trial = fatigue_alldat.(strcat(j,"_stnd"))(i);
                trial = trial{1,1};
                
                %dist
                a = dist([trial, trial_mean]);
                fatigue_alldat.(strcat(j,"_dist"))(i) = a(1,2);
                
                %max
                fatigue_alldat.(strcat(j,"_max"))(i) = max(trial);
                
                %dist2zero
                a = dist([trial, zeros(100000,1)]);
                fatigue_alldat.(strcat(j,"_dist2zero"))(i) = a(1,2);
                
                %correlation
                a = corr([trial, trial_mean]);
                fatigue_alldat.(strcat(j,"_corr"))(i) = a(1,2);
            end
