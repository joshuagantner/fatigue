...
%                                 pipeline = py.list({py.dict(pyargs("$match",filter,"$group",query))});
        ...
        case action == 7 % create table
            disp('create table')
            % input
            emg_space   = emgSpaceSelector(db_name);% set emg space
            % build pipeline
            pipeline = {
                py.dict(pyargs('match', py.dict(pyargs('space', 'test', 'distance', py.dict(pyargs('ne', 'missing leads')))))),
                py.dict(pyargs('lookup', py.dict(pyargs('from', 'parameters', 'localField', 'identifier', 'foreignField', 'identifier', 'as', 'group')))),
                py.dict(pyargs('project', py.dict(pyargs('group', py.dict(pyargs('arrayElemAt', {{'$group', int32(0)}})), 'identifier', int32(1), 'day', int32(1), 'block', int32(1), 'trial', int32(1), 'distance', int32(1)))))
            };
